...
const Loading = () => {
  ...
  useEffect(() => {
    const updateLoadingProgress = () => {
      ...
      const resourcesLoaded = resources.filter(entry => entry.responseEnd > 0).length;
      const progress = Math.min((resourcesLoaded / totalResources) * 100, 100);
      setLoadingProgress(progress);
      if (resourcesLoaded === totalResources && totalResources > 0) {
        setIsLoaded(true);
      }
    };
  ...
  return (
    <div className={`loading-page ${isLoaded ? 'fade-out' : ''}`}>
      ...
      <div className="name-container">
        ...
        <div className="progress-bar" data-label="Loading...">
          <div
