    ...
    const handleFirstClick = () => {
      if (audioRef.current && audioRef.current.paused) {
        audioRef.current.play().then(() => {
          setIsPlaying(true)
        }).catch(error => console.error("Błąd odtwarzania muzyki:", error))
      }
    }

    handleResize()
    window.addEventListener("resize", handleResize)
    document.addEventListener("click", handleFirstClick, { once: true }) // Odpala muzykę tylko raz po pierwszym kliknięciu
