...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleFirstClick = () => {
      if (audioRef.current && !isPlaying) {
        ...
        setIsPlaying(true)
        document.removeEventListener("click", handleFirstClick) // Po pierwszym kliknięciu usuwamy nasłuchiwacz
      }
    }
  ...
  return (
    <nav className={styles.navbar}>
      ...
      <svg className={styles.logoIcon} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
          <path
        ...
        <span className={`${fraunces.className} ${styles.navLogo}`} onClick={isMobile ? toggleMenu : undefined}>
          Mate-Chess
          </span>
      </div>
      ...
      {isMobile && isMenuOpen && (
        <ul className={styles.dropdownMenu}>
          <li className={styles.dropdownItem} onClick={toggleMusic}>
            ...
            <span className={fraunces.className}>{isPlaying ? "Pause Music" : "Play Music"}</span>
          </li>
        </ul>
      )}
    </nav>
