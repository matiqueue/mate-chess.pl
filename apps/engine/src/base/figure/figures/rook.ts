...
class Rook extends Figure {
  ...
  isValidMove(target: Position): boolean {
    ...
    if (this.position.x !== target.x && this.position.y !== target.y) return false
    if (target.figure && target.figure.color === this.color) return false

    const directionX = target.x === this.position.x ? 0 : Math.sign(this.position.x - target.x)
    const directionY = target.y === this.position.y ? 0 : Math.sign(this.position.y - target.y)

    let currentX = target.x + directionX
    let currentY = target.y + directionY
