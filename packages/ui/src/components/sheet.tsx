...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(
  ({ side = "right", className, children, ...props }, ref) => {
    // Sprawdzamy, czy dzieci zawierają już tytuł.
    const hasTitle = React.Children.toArray(children).some(
      (child) =>
        React.isValidElement(child) &&
        // Sprawdzamy na podstawie displayName naszego SheetTitle.
        (child.type as any).displayName === SheetTitle.displayName,
    )
    ...
    return (
      <SheetPortal>
        ...
        <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
          ...
          {/* Jeśli nie znaleziono tytułu, dodajemy domyślny ukryty tytuł */}
          {!hasTitle && <SheetTitle className="sr-only">Dialog</SheetTitle>}
          {children}
